  ...
  <div>
    <header class="head">
      ...
      <div class="secondary" v-show="Showing === false">
        ...
        <form action="">
          <div>
            <input type="text" v-model="message" placeholder="输入 电影名 / 明星 ">
          </div>
        </form>
        <router-link :to="{path:'/search',query:{Message:message}}"><h3 class="close-search" v-show="message"
                                                                        @click="SHOW">搜索</h3></router-link>
        <ul>
          <li>
            <div>
              <span>
    ...
    <context v-show="Showing"></context>
  </div>
</template>
