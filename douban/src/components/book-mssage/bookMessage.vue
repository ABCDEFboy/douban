  ...
  <div>
    ...
    <div class="page" v-show="Showing">
      <div class="card">
        ...
        <div class="subject-info">
          ...
          <div class="left">
            <p class="rating">
              ...
              <span>{{Data.rating.numRaters}}人评论</span>
            </p>
            <p class="meta">
              <span v-for="author in Data.author">{{author}}(作者) / </span>
              <span>{{Data.publisher}}出版社 / </span>
              <span>{{Data.binding}} / </span>
              售价 :{{Data.price}} / 共{{Data.pages}}页
            </p>
          </div>
        </div>
        <div class="subject-intro">
          <h2>{{Data.title}}的内容简介</h2>
          <div class="bd">
        ...
        <section class="subject-comments" v-show="annotations.length>0">
          <h2>{{Data.title}}的评论</h2>
          <div class="bd">
            <ul class="list">
              <li v-for="item in annotations">
                <div class="desc">
                  <img :src="item.author_user.avatar" alt="" width="36px" height="36px">
                  <div class="usr-info">
                    <strong>{{item.author_user.name}}</strong>
...
<script>
  ...
  export default {
    ...
    created() {
      this.$nextTick(() => {
        this.$http.get('/book/' + this.$route.query.id + '')
          .then((res) => {
            this.Data = res.data
        ...
        this.$http.get('/book/' + this.$route.query.id + '/annotations')
          .then((res) => {
